...
def fill_tif_dict(name, sample_year, sample_lat, sample_lon, tif_dict, settings):
    if name == "central":
        ...
        filename = get_input_filename(
            (sample_year,), (sample_lat + settings["tile_len_deg"],), (sample_lon - settings["tile_len_deg"],), settings
        )
    elif name == "north":
...
def read_input_data(settings, tif_dict, sample_year, sample_lon, sample_lat, channels, scene_width):
    ...
    if usecase == "usecase_central":
        central_output = read_tif(
            tif_dict["central"], channels, window=Window.from_slices((ilat_n, ilat_s + 1), (ilon_w, ilon_e + 1))
        )
    ...
    # USECASE 1 - northwest corner
        ...
        central_output = read_tif(
            tif_dict["central"], channels, window=Window.from_slices((0, ilat_s + 1), (0, ilon_e + 1))
        )
    ...
    # USECASE 2 - north edge
        ...
        central_output = read_tif(
            tif_dict["central"], channels, window=Window.from_slices((0, ilat_s + 1), (ilon_w, ilon_e + 1))
        )
        north_output = read_tif(
            tif_dict["north"],
            channels,
            window=Window.from_slices(
                (tif_dict["north_height"] + ilat_n, tif_dict["north_height"]), (ilon_w, ilon_e + 1)
            ),
        )
    ...
    # USECASE 3 - northeast corner
        ...
        north_output = read_tif(
            ...
            channels,
            window=Window.from_slices(
                (tif_dict["north_height"] + ilat_n, tif_dict["north_height"]), (ilon_w, tif_dict["north_width"])
            ),
        )
        ...
        sample_output = np.vstack(
            (np.hstack((north_output, northeast_output)), np.hstack((central_output, east_output)))
        )
    ...
    # USECASE 5 - southeast corner
        ...
        east_output = read_tif(
            ...
            channels,
            window=Window.from_slices(
                (ilat_n, tif_dict["central_height"]), (0, ilon_e - tif_dict["central_width"] + 1)
            ),
        )
        ...
        southeast_output = read_tif(
            ...
            channels,
            window=Window.from_slices(
                (0, ilat_s - tif_dict["central_height"] + 1), (0, ilon_e - tif_dict["central_width"] + 1)
            ),
        )

        sample_output = np.vstack(
            (np.hstack((central_output, east_output)), np.hstack((south_output, southeast_output)))
        )
    ...
    # USECASE 7 - southwest corner
        ...
        west_output = read_tif(
            ...
            channels,
            window=Window.from_slices(
                (ilat_n, tif_dict["west_height"]), (tif_dict["west_width"] + ilon_w, tif_dict["west_width"])
            ),
        )
        south_output = read_tif(
            tif_dict["south"],
        ...
        southwest_output = read_tif(
            ...
            channels,
            window=Window.from_slices(
                (0, ilat_s - tif_dict["central_height"] + 1), (tif_dict["west_width"] + ilon_w, tif_dict["west_width"])
            ),
        )

        sample_output = np.vstack(
            (np.hstack((west_output, central_output)), np.hstack((southwest_output, south_output)))
        )
    ...
    # USECASE 8 - west edge
        ...
        central_output = read_tif(
            tif_dict["central"], channels, window=Window.from_slices((ilat_n, ilat_s + 1), (0, ilon_e + 1))
        )
        west_output = read_tif(
            tif_dict["west"],
            channels,
    ...
    if settings["mode"] == "training":
        random_noise = rng.integers(-settings["input_noise"], settings["input_noise"] + 1, size=1)
