...
def get_model(settings):
    ...
    model = build_model(
        settings,
        input_shape=(settings["scene_width_landsat"], settings["scene_width_landsat"], len(settings["channels"])),
    )
...
def build_model(settings, input_shape):
    ...
    if settings["aug_randomflip"]:
        x = tf.keras.layers.RandomFlip("horizontal_and_vertical", seed=settings["rng_seed"])(scaled_input)
    else:
        x = scaled_input
    ...
    # Final layer here
    x = tf.keras.layers.Dense(1, bias_initializer=tf.keras.initializers.Zeros(), activation=FINAL_ACTIVATION)(x)
    x = tf.keras.layers.Rescaling(scale=100.0, offset=0.0)(x)
